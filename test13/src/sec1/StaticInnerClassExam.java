...
class Outer2 {	// 외부 클래스
	...
	static class Inner {	// 정적 내부 클래스
		...
		void inMethod() {
			System.out.println("내부에서 내부 클래스의 인스턴스 변수 : " + inNum);
			System.out.println("내부에서 내부 클래스의 정적 멤버 변수 : " + sInNum);
...
//		System.out.println("외부에서 내부 클래스의 인스턴스 변수 : " + inNum); // 
		System.out.println("외부에서 내부 클래스의 정적 멤버 변수 : " + Inner.sInNum);
...
//		out.outMethod();	// outMethod()는 정적 메소드이므로 객체를 생성하지 않고 실행
		Outer2.outMethod();
//		Outer2.Inner.inMethod();	// inMethod()는 인스턴스 메소드기 때문에 이렇게 호출은 불가능
//		out.Inner.inMethod();	// Inner가 정적 클래스이므로 out 인스턴스를 생성하지 않고, Out 
