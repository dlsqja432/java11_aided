...
// Matcher : 특정 패턴의 형태가 맞는 지 비교하는 클래스
// 정규식(Reg Expression) => RegEx
public class PatternExam1 {

	public static void main(String[] args) {
		String[] data = {"bag", "kim1004", "96", "8to5", "single", "Mom", "ABC", "장인범", "^_^"};
...
//		Pattern p = Pattern.compile("e$");			// 특정 문자로 끝나는 경우 문자 뒤에 $ (웹만 가능)
//		Pattern p = Pattern.compile("^s");			// 특정 문자로 시작하는 경우 문자 앞에 ^ (웹만가능)
		Pattern p = Pattern.compile("[0-9]*");		// 모든 값이 숫자인 데이터
	
		for(int i=0; i<data.length; i++) {			
			Matcher m = p.matcher(data[i]);
			if(m.matches()) {
				System.out.println("형식 일치 : " + data[i]);
			}
		}
	}
}
